...
class LSEGen<T> {
	...
	public boolean insere(int pos, T dado) {
		if ((vazia()) && (pos != 1)){
	        return false; /* lista vazia mais posicao inv*/
	    }
...
public class ADAINDEX {
    ...
    public static void main(String[] args) {
        ...
        int n, q;
        int contador = 0;
        String aux, input, aux2;
        LSEGen palavra = new LSEGen();
        LSEGen prefixo = new LSEGen();
        ...
        q = Integer.parseInt(numeros[1]);
        
        //Okay, agora vamos pegar o numero de palavras salvos em n e colocar em uma lista encadeada
            for(int i = 1; i <= n; i++){ //A lista só começa da primeira posição
                input = leitor.nextLine();
        ...
        //Bom, temos ambas as paradas salvas, vamos
            for(int i = 1; i<=q; i++){ //A lista só começa da primeira posição
                aux = (String) prefixo.elemento(i);
